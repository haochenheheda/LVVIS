...
class LVVISeval:
    # Interface for evaluating video instance segmentation on the LVVIS dataset.
    #
    # The usage for LVVISeval is as follows:
    #  cocoGt=..., cocoDt=...       # load dataset and results
    #  E = LVVISeval(cocoGt,cocoDt); # initialize YTVOSeval object
    ...
    def _prepare(self):
        ...
        def _toMask(anns, coco):
            ...
            for ann in anns:
                ...
                l = [int(a) for a in ann['areas'] if a]
                if len(l)==0:
                  ann['avg_area'] = 0
                else:
    ...
    def evaluateVid(self, vidId, catId, aRng, maxDet):
        ...
        for g in gt:
            if (g['avg_area']<aRng[0] or g['avg_area']>aRng[1]):
                g['_ignore'] = 1
            else:
                g['_ignore'] = 0
    ...
    def summarize(self):
        ...
        def _summarize( ap=1, iouThr=None, areaRng='all', maxDets=100 ):
            ...
            if ap == 1:
                ...
                s = s[:,:,aind,mind]
            if ap == 1:
                bs = s[:,:,np.array(self.cocoGt.base_categories)-1]
                ns = s[:,:,np.array(self.cocoGt.novel_categories)-1]
            else:
                bs = s[:,np.array(self.cocoGt.base_categories)-1]
                ns = s[:,np.array(self.cocoGt.novel_categories)-1]
            ...
            if len(bs[bs>-1])==0:
                ...
                mean_bs = np.mean(bs[bs>-1])

            if len(ns[ns>-1])==0:
                mean_ns = -1
            else:
                mean_ns = np.mean(ns[ns>-1])

            print(iStr.format(titleStr, typeStr, iouStr, areaRng, maxDets, mean_s))
            print(iStr.format(titleStr, typeStr + 'b', iouStr, areaRng, maxDets, mean_bs))
            print(iStr.format(titleStr, typeStr + 'n', iouStr, areaRng, maxDets, mean_ns))
            return mean_s
        def _summarizeDets():
            stats = np.zeros((12,))
