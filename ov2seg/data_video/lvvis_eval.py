...
from .datasets.ytvis_api.lvvis import LVVIS
...
class LVVISEvaluator(DatasetEvaluator):
    """
    ...
    def __init__(
        ...
        *,
        ...
        with contextlib.redirect_stdout(io.StringIO()):
            self._ytvis_api = LVVIS(json_file)
    ...
    def evaluate(self):
        ...
        if self._output_dir:
            PathManager.mkdirs(os.path.join(self._output_dir, self.dataset_name))
            file_path = os.path.join(self._output_dir, self.dataset_name, "results.json")
...
def instances_to_coco_json_video(inputs, outputs):
    ...
    image_size, height, width, mid_image_size = outputs[0]["interpolate_meta"]
    outputs = outputs[0]['instances']

    scores = outputs.scores.cpu().numpy()
    labels = outputs.pred_classes.cpu().numpy()
    masks = outputs.pred_masks
    
    ytvis_results = []
    for instance_id, (s, l, m) in enumerate(zip(scores, labels, masks)):
        m = sem_seg_postprocess(m[None,...], image_size, height, width, mid_image_size)[0]
        m = (m > 0).float().numpy()
        segms = [
...
def sem_seg_postprocess(result, img_size, output_height, output_width, mid_image_size):
    """
    ...
    for i in range(0, result.shape[1], window_size):
        sliced_mid_result.append(F.interpolate(
            result[:,i:i+window_size], size=mid_image_size, mode="bilinear", align_corners=False
        ))
    result = torch.cat(sliced_mid_result, dim=1)
